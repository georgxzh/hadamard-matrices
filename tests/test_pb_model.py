...
def test_translation_canonical_model_counts_and_accepts_normalized_witness() -> None:
    ...
    left, _ = canonical_residue_translation(left, 3)
    right, _ = canonical_residue_translation(right, 3)

    model = UncompressionPBModel(first, second, 9, canonical_translations=True)
    assert model.stats.symmetry_inequalities == 16
    assert model.stats.constraint_records == 2_843
    assert model.stats.normalized_inequalities == 2_862
    assert model.first_failed_constraint(left, right) is None
    assert residue_zero_bits(left, 3) == min(
        residue_zero_bits(left, 3)[step:] + residue_zero_bits(left, 3)[:step]
        for step in range(9)
    )


def test_translation_canonical_p37_model_counts() -> None:
    first, second = structured_compressed_pair(37, 3)
    model = UncompressionPBModel(first, second, 9, canonical_translations=True)
    assert model.stats.variables == 111_222
    assert model.stats.symmetry_inequalities == 16
    assert model.stats.constraint_records == 442_480
    assert model.stats.normalized_inequalities == 442_720
